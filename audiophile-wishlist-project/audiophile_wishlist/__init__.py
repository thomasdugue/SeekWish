...
class Plugin(BasePlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._downloaded = set()        # (username, virtual_path) pairs to avoid dupes
        self._stats = {
            "filtered_out": 0,
            "quality_matched": 0,
            "downloaded": 0,
    ...
    def _on_search_response(self, msg):
        """Called for every file search response. Filters and auto-downloads.

        This hooks into the N+ events system (non-standard for plugins).
        It runs AFTER the core search handler processes the response.
        """

        if not self.settings["enable_quality_filter"]:
            return

        # Skip rejected responses
        if msg.token is None or msg.list is None:
            return

        # Check if this response is for a wishlist search
        try:
        ...
        # Only process wishlist searches
        from pynicotine.search import WishSearchRequest
        if not isinstance(search, WishSearchRequest):
            return

        username = getattr(msg, "username", None) or getattr(msg, "search_username", None)
        if not username:
            return

        # Evaluate each file in the response
        best_match = None
        best_score = -1

        for fileinfo in msg.list:
            # fileinfo format: (code, filepath, size, ext, attrs)
            if len(fileinfo) < 5:
                continue
            ...
            if (username, filepath) in self._downloaded:
                continue

            passes, reason = _check_quality(filepath, file_size, file_attrs or {}, self.settings)
            ...
            # Score this result to pick the best one
            score = _score_result(filepath, file_size, file_attrs or {})
            if score > best_score:
                best_score = score
                best_match = (username, filepath, file_size, file_attrs)

        # Also check private results if available
        if hasattr(msg, "privatelist") and msg.privatelist:
            for fileinfo in msg.privatelist:
                if len(fileinfo) < 5:
                    continue
                ...
                passes, _reason = _check_quality(filepath, file_size, file_attrs or {}, self.settings)
                if not passes:
                    self._stats["filtered_out"] += 1
                    continue
        ...
        b_user, b_path, b_size, b_attrs = best_match
        ...
        if mode == "auto":
            # Auto-download: enqueue the file directly
            try:
                self.core.downloads.enqueue_download(b_user, b_path, size=b_size, file_attributes=b_attrs)
                self._downloaded.add((b_user, b_path))
                self._stats["downloaded"] += 1
                self.log(
                    "Auto-downloaded: %s from %s [%s %s/%sbit %skbps]",
                    (os.path.basename(b_path.replace("\\", "/")),
                     b_user, ext.upper(), sr, bd, br),
                )
            except Exception as e:
                self.log("Error enqueuing download: %s", (str(e),))

        elif mode == "semi":
            # Semi-auto: enqueue paused for manual review
            try:
                self.core.downloads.enqueue_download(
                    b_user, b_path, size=b_size, file_attributes=b_attrs, paused=True,
                )
                self._downloaded.add((b_user, b_path))
                self._stats["downloaded"] += 1
                self.log(
                    "Queued (paused): %s from %s [%s %s/%sbit %skbps]",
                    (os.path.basename(b_path.replace("\\", "/")),
                     b_user, ext.upper(), sr, bd, br),
                )
            except Exception as e:
